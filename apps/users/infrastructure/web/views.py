...
from apps.users.application.selectors.delate_user_by_id import delete_users_by_id
from apps.users.application.selectors.get_users_by_type import get_users_by_type_list
...
def list_create_users_view(request):
  ...
  if request.method == "GET":
      ...
      try:
          ...
          return Response(
              {"error": f"El correo '{data['correo']}' ya existe."},
              status=status.HTTP_409_CONFLICT,
          )
      except Exception as e:
          return Response(
              {"error": f"Error inesperado al crear usuario: {str(e)}"},
              status=status.HTTP_500_INTERNAL_SERVER_ERROR,
          )
...
def user_detail_update_delete_view(request, correo: str):
  ...
  if request.method == "GET":
      ...
      try:
          ...
          return Response(
              {"error": f"Error inesperado al actualizar: {str(e)}"},
              status=status.HTTP_500_INTERNAL_SERVER_ERROR,
          )
      ...
      try:
          ...
          if not deactivated_user:
              return Response(
                  {"error": "Usuario no encontrado para deshabilitar"},
                  status=status.HTTP_404_NOT_FOUND,
              )
          ...
          return Response(
              {"error": f"Error inesperado al deshabilitar: {str(e)}"},
              status=status.HTTP_500_INTERNAL_SERVER_ERROR,
          )
...
class MeView(APIView):
  ...
  def get(self, request):
      ...
      if isinstance(claims, dict):
          ...
          data["tipo_usuario_param"] = data["tipo_usuario_param"] or claims.get(
              "tipo_usuario_param"
          )
...
# Nuevo endpoint: perfil completo del usuario autenticado
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def user_profile_completo_view(request):
  """
  Endpoint para obtener el perfil COMPLETO del usuario autenticado.
  Usa el ID del JWT para ligar contra la tabla de institución vía selector.
  """
  try:
      ...
      from apps.users.application.selectors.get_user_by_id import (
          get_user_profile_completo,
      )
