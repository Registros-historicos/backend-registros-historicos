...
class BulkImpiService:
    ...
    def execute(self, file, id_usuario: int, hojas_input: str = None):
        ...
        return {
            ...
            "errores_detalle": {"autores": errores_autores, "registros": errores_registros},
        }
    ...
    def _procesar_autores(self, xls):
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                errores.append({"fila": int(index) + 2, "curp": curp, "error": str(e)})
    ...
    def _procesar_registros(self, xls, id_usuario, hojas_input):
        ...
        for hoja in hojas_a_cargar:
            ...
            for index, row in df.iterrows():
                expediente = str(
                    row.get("N. Expediente(1)", "")
                    or row.get("N. Expediente (1)", "")
                    or row.get("Expediente", "")
                ).strip()
                if not expediente:
                    continue
                ...
                try:
                    ...
                    titulo = str(
                        row.get("Título (2)", "")
                        or row.get("N. De Título (2)", "")
                        or row.get("Titulo (2)", "")
                    ).strip() or None

                    descripcion = str(
                        row.get("Descripción (3)", "")
                        or row.get("Denomicación (3)", "")
                        or row.get("Denominación (3)", "")
                    ).strip() or None

                    inventores = str(
                        row.get("Autores (14)", "")
                        or row.get("Inventores (14)", "")
                    ).strip()
                    ...
                    registro_data = {
                        ...
                        "no_titulo": str(row.get("N. de Certificado (5)", "")).strip()
                            or str(row.get("N. De Título (2)", "")).strip() or None,
                        "estatus_param": self._to_int(row.get("Estatus (6)") or row.get("Estatus (6)")),
                        "rama_param": self._to_int(row.get("Rama (5)") or row.get("Rama (7)")),
                        "medio_ingreso_param": 18,  # Tema 4 → Método de ingreso
                        "tipo_ingreso_param": 38,   # Tema 8 → Tipo de ingreso
                        "tipo_registro_param": 44,  # Tema 9 → Tipo de registro IMPI
                        "tecnologico_origen": str(row.get("Tecnologico de Origen (8)", "")).strip(),
                        "anio_renovacion": self._to_int(row.get("Año Renovación (10)")
                                                       or row.get("Año renovación (10)")),
